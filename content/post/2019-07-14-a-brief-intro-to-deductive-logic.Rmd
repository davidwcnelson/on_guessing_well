...
Notice that we use the $\equiv$ sign rather than $=$, as we mean "equivalent by definition", that is $P_1$ actually stands for $``\text{My favorite color is purple.''}$, or more precisely they have the same "truth value". Also note that $P_1$ is an opinion (held by me), rather than some universal truth.
...
Interestingly, we don't actually need to define conjunction to use it: it can be derived from disjunction and negation. Even more interestingly, is that all three logical operators can be derived from one of two single operators, referred to as [NAND](https://en.wikipedia.org/wiki/Sheffer_stroke) (not both) with the symbol $\uparrow$, and [NOR](https://en.wikipedia.org/wiki/Logical_NOR) (not either) with the symbol $\downarrow$. We're not going to get into concept of "[functional completeness](https://en.wikipedia.org/wiki/Functional_completeness)", but it is really cool and I highly suggest you check it out.

Before we go any further, we should really talk about "[Truth Tables](https://en.wikipedia.org/wiki/Truth_table)". Truth tables are an incredibly useful tool in deductive logic that let us understand the relationships between premises given all possible truth values of the premises. What follows is a truth table for two arbitrary propositions $P$ and $Q$ and their conjunction $P \wedge Q$. Here $T$ represents a truth value of "True" and $F$ stands for a truth value of "False":
...
Note that the converse is not true i.e. that is if we know $Q$, we cannot guarantee that $P$ is also true. This is because $P$ can be true or false given that $Q$ is true. This can be seen via the truth table for implication. This is a [formal fallacy](https://en.wikipedia.org/wiki/Formal_fallacy) (i.e. error of form) referred to as "[Affirming the Consequent](https://en.wikipedia.org/wiki/Affirming_the_consequent)". This fallacy is the very premise of "[inductive reasoning](https://en.wikipedia.org/wiki/Inductive_reasoning)" where the premises do not guarantee the conclusion, they only make the conclusion more plausible. We will have a lot more to say of this in the future.

A deductive argument that suffers from an error of form is said to be "invalid". An argument whose conclusions are guaranteed by its premises is said to be valid. A deductive argument that is valid, but whose premises are false is referred to as "unsound", whereas an argument whose premises are true is referred to as "sound." The soundness of an argument cannot necessarily be guaranteed by form of the argument! This is an important issue that has major implications for how we reason in the real world.  
